...
def get_model_extraction_path(model_id: str, model_version: str) -> str:
    """
    Get the path where the model should be extracted.
    Extracts next to automl_predictor directory.
    ...
    # Go up one level to main-data-retrieval-endpoint
    main_dir = os.path.dirname(script_dir)
    # Create extraction path next to automl_predictor
    extract_dir = os.path.join(main_dir, f"automl_predictor_{model_id}_{model_version}")
    return extract_dir


def call_explain_model_endpoint(model_path: str, data_file_path: str, 
                                target_col: str = "signature", 
                                max_shap_samples: int = 100,
                                output_file: str = "report.html",
                                endpoint_url: str = "http://localhost:5005/explain-model") -> str:
    """
    Call the explain-model endpoint after model extraction.
    
    Args:
        model_path: Path to the extracted model directory
        data_file_path: Path to the CSV data file
        target_col: Target column name (default: "signature")
        max_shap_samples: Maximum SHAP samples (default: 100)
        output_file: Output HTML report file path
        endpoint_url: Explain model endpoint URL
    ...
    try:
        logger.info("=" * 80)
        logger.info("Calling explain-model endpoint...")
        logger.info(f"  Model path: {model_path}")
        logger.info(f"  Data file: {data_file_path}")
        logger.info(f"  Target column: {target_col}")
        logger.info(f"  Max SHAP samples: {max_shap_samples}")
        ...
        with open(data_file_path, 'rb') as data_file:
            files = {
                'data_file': (os.path.basename(data_file_path), data_file, 'text/csv')
            }
            data = {
                'model_path': model_path,
                'target_col': target_col,
                'max_shap_samples': str(max_shap_samples)
            }
            
            # Make the POST request
            logger.info("Sending POST request to explain-model endpoint...")
            ...
            logger.info(f"✅ Explainability report generated successfully!")
            logger.info(f"   Report saved to: {output_path}")
            logger.info(f"   Response status: {response.status_code}")
            logger.info(f"   Report size: {len(response.content)} bytes")
            
            return output_path
            
    except requests.exceptions.RequestException as e:
        logger.error(f"Failed to call explain-model endpoint: {e}")
        if hasattr(e, 'response') and e.response is not None:
            logger.error(f"Response status: {e.response.status_code}")
            logger.error(f"Response body: {e.response.text[:500]}")
        raise
    except Exception as e:
        logger.error(f"Error calling explain-model endpoint: {e}", exc_info=True)
        raise
...
async def process_xai_trigger(event: dict) -> None:
    ...
    try:
        ...
        try:
            ...
            # Download model ZIP file from endpoint
            model_bytes = download_model_file(user_id, model_id, model_version)
            logger.info(f"Model downloaded: {len(model_bytes)} bytes")
            
            # Always extract the ZIP file (endpoint always returns ZIP)
            logger.info("Extracting model ZIP file...")
            extract_path = get_model_extraction_path(model_id, model_version)
            logger.info(f"Extraction path: {extract_path}")
            
            # Remove existing directory if it exists
            if os.path.exists(extract_path):
                import shutil
                logger.info(f"Removing existing directory: {extract_path}")
                shutil.rmtree(extract_path)
            
            model_extracted_files = extract_model_folder(model_bytes, extract_path)
            logger.info(f"Extracted {len(model_extracted_files)} model files to {extract_path}:")
            for file_path in model_extracted_files[:10]:  # Show first 10 files
                logger.info(f"  - {file_path}")
            if len(model_extracted_files) > 10:
                logger.info(f"  ... and {len(model_extracted_files) - 10} more files")
            logger.info(f"✅ Model extracted successfully to: {extract_path}")
            
            # Step 2.5: Call explain-model endpoint immediately after extraction
            try:
                # Get the main directory path
                script_dir = os.path.dirname(os.path.abspath(__file__))
                main_dir = os.path.dirname(script_dir)
                data_file_path = os.path.join(main_dir, "csv-pkl-json", "mvp-data.csv")
                
                # The extracted ZIP contains automl_predictor-1 subdirectory
                # Append it to the extract path
                actual_model_path = os.path.join(extract_path, "automl_predictor-1")
                
                # Verify the subdirectory exists
                if not os.path.exists(actual_model_path):
                    logger.warning(f"Expected model subdirectory not found: {actual_model_path}")
                    logger.info("Checking extracted files for model directory...")
                    # Try to find the model directory
                    for file_path in model_extracted_files[:20]:
                        if 'automl_predictor' in file_path and os.path.isdir(file_path):
                            actual_model_path = file_path
                            logger.info(f"Found model directory: {actual_model_path}")
                            break
                    else:
                        logger.error("Could not find automl_predictor-1 subdirectory in extracted files")
                        raise FileNotFoundError(f"Model subdirectory not found in {extract_path}")
                
                # Convert Windows path to format expected by endpoint (forward slashes)
                model_path_formatted = actual_model_path.replace('\\', '/')
                logger.info(f"Using model path: {model_path_formatted}")
                
                report_path = call_explain_model_endpoint(
                    model_path=model_path_formatted,
                    data_file_path=data_file_path,
                    target_col="signature",
                    max_shap_samples=100,
                    output_file=f"report_{model_id}_{model_version}.html"
                )
                logger.info(f"✅ Explainability report ready: {report_path}")
                
            except Exception as e:
                logger.error(f"Failed to generate explainability report: {e}", exc_info=True)
                # Don't return here - continue with the rest of the process
            
        except Exception as e:
            logger.error(f"Failed to fetch/download/extract model: {e}", exc_info=True)
            return
        ...
        if extract_path and model_extracted_files:
            logger.info(f"  Model: {len(model_extracted_files)} files extracted to {extract_path}")
            logger.info(f"  Model path available for XAI: {extract_path}")
        else:
            logger.warning("  Model extraction path not available")
        # Example: Load model from extracted path
        # For AutoGluon models, use the directory path directly:
        # from autogluon.tabular import TabularPredictor
        # predictor = TabularPredictor.load(extract_path)
        # 
        # For pickle models:
        # import pickle
        # model_pkl = [f for f in model_extracted_files if f.endswith('model.pkl') or f.endswith('predictor.pkl')][0]
